...
class SaveIf {
 public:
  virtual ~SaveIf() {}
  virtual int32_t save_data(const std::string& username, const std::string& password, const int32_t aid, const int32_t bid) = 0;
};
...
class SaveNull : virtual public SaveIf {
 public:
  virtual ~SaveNull() {}
  int32_t save_data(const std::string& /* username */, const std::string& /* password */, const int32_t /* aid */, const int32_t /* bid */) override {
    int32_t _return = 0;
    return _return;
  }
};
...
typedef struct _Save_save_data_args__isset {
  _Save_save_data_args__isset() : username(false), password(false), aid(false), bid(false) {}
  bool username :1;
  bool password :1;
  bool aid :1;
  bool bid :1;
...
class Save_save_data_args {
 public:
  ...
  void __set_aid(const int32_t val);
...
class SaveClient : virtual public SaveIf {
 ...
 public:
  ...
  int32_t save_data(const std::string& username, const std::string& password, const int32_t aid, const int32_t bid) override;
  void send_save_data(const std::string& username, const std::string& password, const int32_t aid, const int32_t bid);
...
class SaveMultiface : virtual public SaveIf {
 ...
 public:
  int32_t save_data(const std::string& username, const std::string& password, const int32_t aid, const int32_t bid) override {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->save_data(username, password, aid, bid);
    }
    return ifaces_[i]->save_data(username, password, aid, bid);
  }
  ...
  SaveConcurrentClient(std::shared_ptr< ::apache::thrift::protocol::TProtocol> iprot, std::shared_ptr< ::apache::thrift::protocol::TProtocol> oprot, std::shared_ptr< ::apache::thrift::async::TConcurrentClientSyncInfo> sync) : sync_(sync)
{
 ...
 public:
  ...
  int32_t save_data(const std::string& username, const std::string& password, const int32_t aid, const int32_t bid) override;
  int32_t send_save_data(const std::string& username, const std::string& password, const int32_t aid, const int32_t bid);
